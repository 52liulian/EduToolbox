...
(function () {
  ...
  /**
   * 全屏
   * 说明：舞台（#teamsStage）右上角工具栏的 ⛶ 由共享模块 tool-stage-toolbar.js
   *       接管，本工具不再自行调用 Fullscreen API。
   *       本工具没有可隐藏的设置栏（状态栏与历史记录不在舞台内），故不接 ⚙。
   */
  ...
  // 舞台右上角工具栏：⛶ 队伍卡片区全屏（panelHost: null —— 本工具无可隐藏的设置栏）
  if (window.EduToolStageToolbar) {
    window.EduToolStageToolbar.init({ stage: '#teamsStage', panelHost: null, hiddenClass: 'setup-hidden' });
  }
