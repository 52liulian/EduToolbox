 ...
 * - 全屏专注模式（覆盖层 + ⛶ 由 assets/js/tool-stage-toolbar.js 统一接管）
 * - localStorage 持久化保存所有定时器配置与剩余时间
 * - 配色主题由站点顶栏全局开关（#accentBtn / et-accent）控制，工具内不再切换
 *
...
(function () {
  ...
  /* 配色主题已交由站点顶栏的全局开关（#accentBtn / localStorage 键 et-accent）
     统一切换，本工具不再维护本地主题状态与持久化。 */
   ...
   * 进入全屏专注模式：显示自定义覆盖层
   *
   * 真正的 requestFullscreen 由共享模块 assets/js/tool-stage-toolbar.js 发起，
   * 全屏目标就是 #focusOverlay 自身（⛶ 按钮点击回调注册在前，先显示覆盖层再
   * 请求全屏，保证请求时元素已可见）。退出时由 fullscreenchange 回调收起覆盖层。
  ...
  function bindEvents() {
    ...
    // ⛶ 全屏专注：显示覆盖层；真正的 requestFullscreen 由共享模块发起
  ...
  /**
   * 入口：加载持久化数据 → 渲染 → 绑定事件 → 接入舞台工具栏 → 启动主循环
   */
  function init() {
    timers = load();
    bindEvents();
    ...
    /* 舞台右上角工具栏（⛶ 全屏）：全屏目标是 #focusOverlay 自身。
       ⚠️ 工具栏本体挂在 main.app 内（覆盖层默认 display:none，放进去就点不到），
          模块只按 id 找按钮、不要求按钮在 stage 内，因此两者分离是安全的。
       本工具没有可隐藏的设置栏，panelHost 传 null。 */
    if (window.EduToolStageToolbar) {
      window.EduToolStageToolbar.init({ stage: '#focusOverlay', panelHost: null });
    }
