...
(function () {
  ...
  function stop() {
    ...
    meterPanel.classList.remove("danger-overlay");
  ...
  function loop() {
    ...
    if (db >= th) {
      ...
      meterPanel.classList.add("danger-overlay");
  ...
  /* 舞台工具栏：⛶ 对 #meterPanel 自身全屏（本工具无可隐藏的设置栏，不接 ⚙）。
     交互与全屏状态回滚均由共享模块 tool-stage-toolbar.js 提供。 */
  if (window.EduToolStageToolbar) {
    window.EduToolStageToolbar.init({ stage: "#meterPanel", panelHost: null });
  }
})();
