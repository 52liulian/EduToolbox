...
(function () {
  ...
  var fullscreenBody = $("fullscreenBody");
  ...
  function enterFullscreen() {
    ...
    // 用 KaTeX 直接渲染到全屏容器，避免克隆 svg 引用
    try {
      katex.render(input.value, fullscreenBody, {
        displayMode: true,
        throwOnError: false,
        errorColor: "#dc2626",
        output: "html"
      ...
      fullscreenBody.textContent = "渲染失败：" + (e && e.message ? e.message : String(e));
    }
    fullscreenOverlay.hidden = false;
    document.body.classList.add("fe-body-fullscreen");
    $("btnExitFullscreen").hidden = false;
  }
  ...
  function exitFullscreen() {
    ...
    fullscreenBody.innerHTML = "";
    document.body.classList.remove("fe-body-fullscreen");
    $("btnExitFullscreen").hidden = true;
  }
