...
(function () {
  ...
  /**
   * 取主题宿主元素：倒计时舞台 #screen
   * 说明：10 套配色是本工具的「业务配色」，一律挂在工具自身舞台元素上，
   *       不写 document.body / documentElement —— 站点级配色（html data-theme /
   *       et-accent）只由站点顶栏 🎨 全局切换，避免令牌空间撞车。
   * @returns {HTMLElement|null} 舞台元素；缺失时返回 null（调用方静默降级）
   */
  function themeHost() { return el.screen || null; }
  ...
  function applyTheme(theme) {
    ...
    for (var i = 0; i < THEMES.length; i++) {
      host.classList.remove('theme-' + THEMES[i]);
    }
    if (theme === 'auto') {
      // 自动渐变：立刻应用第一个非 auto 主题
      autoThemeIndex = 0;
      host.classList.add('theme-' + THEMES[1 + autoThemeIndex]);
      startAutoTheme();
    } else {
      stopAutoTheme();
  ...
  function startAutoTheme() {
    ...
    autoThemeTimer = setInterval(function () {
      // 移除当前主题类
      host.classList.remove('theme-' + THEMES[1 + autoThemeIndex]);
   ...
   * 八、全屏模式
   *
   * ⚠️ 全屏的「切换动作」由共享模块 EduToolStageToolbar 接管：目标是舞台
   *    #screen 自身（旧版用 documentElement，挂载态会被 Chrome 提升到影子
   *    宿主，全屏的是外层白色容器）。fullscreenchange 的监听也由共享模块在
   *    shim 与真实 document 双注册 —— 本工具不再自己监听，避免重复打架。
   * ========================================================== */
  ...
  function cacheDom() {
    ...
    el.slogan = $('slogan');
    el.settingsBtn = $('hideSetupBtn');
    el.fullscreenBtn = $('fullscreenBtn');
   ...
   */
  function bindEvents() {
    // ⚙ 打开 / 关闭设置浮层（全屏时舞台外的内容不渲染，先退出全屏再打开）
    el.settingsBtn.addEventListener('click', function () {
      if (isFullscreen()) exitFullscreenQuietly();
      if (el.overlay.classList.contains('show')) closeSettings();
      else openSettings();
    });
    // ⛶ 全屏由共享模块接管；这里只补一条「进入全屏时收起设置浮层」
    el.fullscreenBtn.addEventListener('click', closeSettings);
  ...
  function init() {
    ...
    /* 舞台右上角工具栏：全屏目标是舞台 #screen 自身；本工具没有可隐藏的
       设置栏（设置是居中浮层），故 panelHost 传 null，只接全屏不做显隐。
