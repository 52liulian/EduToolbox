...
(function () {
   ...
   * assets/js/tool-stage-toolbar.js 统一负责，本文件只保留全屏切换后的重测量。 */

  /**
   * 全屏状态变化处理：重新测量舞台尺寸并重新定位文本。
   *
   * 注意：设置栏的「进入全屏自动隐藏 / 退出全屏还原 / 请求失败回滚」已统一交给
   * 共享模块 assets/js/tool-stage-toolbar.js（init 在下方 init() 末尾调用），
   * 这里**不再**操作 panel 的显隐，避免与共享模块重复维护同一份状态。
   * @returns {void}
   */
  ...
  // 隐藏 / 显示设置栏（舞台右上角齿轮）与全屏（⛶）由共享模块接管，见 init() 末尾
